...
function App() {
  ...
  useEffect(() => {
    const checkMetaMaskConnection = async () => {
      ...
      try {
        // Check existing connections without prompting user
  ...
  // Connect wallet when user clicks button
  ...
  // Clear wallet connection from app
  const disconnectWallet = async () => {
    try {
      // Try to reset MetaMask permissions
  ...
  // Fetch user's NFTs - tries multiple methods for compatibility
  ...
  const buyNFT = async (tokenId, price) => {
    ...
    try {
      ...
      const tx = await contract.buyNFT(tokenId, {
        value: priceInWei,
        gasLimit: gasEstimate ? Math.floor(Number(gasEstimate) * 1.2) : 500000
      });
  ...
  const handleListNFT = async () => {
    ...
    try {
      ...
      if (existingListing.seller !== "0x0000000000000000000000000000000000000000" && 
    ...
    <div className="app gradient-bg">
      <header className="header">
        ...
        <nav className="tabs">
          ...
          <button 
            className={`tab ${activeTab === "my-nfts" ? "active" : ""}`}
            ...
            <div className="nft-grid">
              {!account ? (
                <div className="empty-state">
                  ...
                  <button onClick={connectWallet} className="material-btn primary">
                    🦊 Connect Wallet
                  </button>
                </div>
                        ...
                        {nft.isListed ? (
                          <div className="listed-status">
                            <span className="badge success">
                              📈 Listed for {nft.listingPrice} ETH
                            </span>
                            <button
                              onClick={() => cancelListing(nft.tokenId)}
                              disabled={loading}
